...
for K=2:N     %%%%%%%%%Time loop
    for J=2:(M-1) %%Space loop
        ...
        S(K,J)=S(K-1, J)+ dt/(dx)^2*ds*( S(K-1,J-1)-2*S(K-1, J) +S(K-1, J+1))-dt*(2+ ((J-1)*dx-0.5)^2 )*S(K-1,J)*I(K-1,J)/( S(K-1,J)+I(K-1,J)  ) +dt*2* I(K-1,J); 

        I(K,J)=I(K-1, J)+ dt/(dx)^2*di*( I(K-1,J-1)-2*I(K-1, J) +I(K-1, J+1))+dt*(2+ ((J-1)*dx-0.5)^2 )*S(K-1,J)*I(K-1,J)/( S(K-1,J)+I(K-1,J)  ) -dt*2* I(K-1,J);     
