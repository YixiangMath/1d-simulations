...
for K=2:N     %%%%%%%%%Time loop
    for J=2:(M-1) %%Space loop
        ...
        S(K,J)=S(K-1, J)+ dt/(dx)^2*ds*( S(K-1,J-1)-2*S(K-1, J) +S(K-1, J+1))-dt*2*S(K-1,J)*I(K-1,J) +dt*( 4-pi*sin(pi* (J-1)*dx) )* I(K-1,J); 

        I(K,J)=I(K-1, J)+ dt/(dx)^2*di*( I(K-1,J-1)-2*I(K-1, J) +I(K-1, J+1))+dt*2*S(K-1,J)*I(K-1,J) -dt*( 4-pi*sin(pi* (J-1)*dx) )* I(K-1,J);     
...
legend('S(x,10)', 'I(x,10)')
 ...
 saveas(h, '/Users/ywu/Library/Mobile Documents/com~apple~CloudDocs/My Papers/2023-3-degenerate SIS/1d simulations/massDS2', 'epsc');
% subplot(1,2,1)
