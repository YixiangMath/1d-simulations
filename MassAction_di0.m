...
plot(x, u1(T+1,:),'LineWidth',2)
hold on

plot(x, u2(T+1,:),'LineWidth',2)

xlabel('x')

legend('S(x,50)', 'I(x,50)')

saveas(h, '/Users/ywu/Library/Mobile Documents/com~apple~CloudDocs/Work/My Papers/2025-9-KongFanze-SIS mass action spikes/massDI0', 'epsc');
...
c = [1; 1];                                  
f = [1; 0] .* DuDx;                   
F=0.2*u(1).*u(2)-(4-pi*sin(pi*x)).*u(2);
%y = u(1) - u(2);
